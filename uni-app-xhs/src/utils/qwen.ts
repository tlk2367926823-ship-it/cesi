...
function getRecentDrafts(): Array<Pick<ShareDraft, "title" | "body">> {
  try {
    const raw = uni.getStorageSync(DRAFT_HISTORY_KEY);
...
function rememberDraft(draft: ShareDraft) {
  ...
  uni.setStorageSync(DRAFT_HISTORY_KEY, JSON.stringify(nextHistory));
}

function getNextFallbackDraft(): ShareDraft {
  const current = Number(uni.getStorageSync(FALLBACK_INDEX_KEY) || "0");
  const next = current + 1;
  uni.setStorageSync(FALLBACK_INDEX_KEY, String(next));
